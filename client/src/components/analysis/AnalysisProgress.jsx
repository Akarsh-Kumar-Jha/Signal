  ...
  return (
    <div className="max-w-xl mx-auto px-2 sm:px-4 py-6 sm:py-12">
      <div className="panel-brutal p-3.5 sm:p-8 space-y-4 sm:space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between pb-3 border-b-2 border-[#18181B] font-mono gap-2">
          <div className="flex items-start sm:items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="font-display text-xs sm:text-sm font-extrabold text-[#18181B] break-words line-clamp-2 leading-tight">
                "{userQuery}"
              </h2>
            </div>
          </div>
        ...
        <div className="space-y-2.5 font-mono text-xs">
          {PIPELINE_STEPS.map((step, idx) => {
            ...
            return (
              <div
                key={step.id}
                className={`p-2.5 sm:p-3 border-2 border-[#18181B] flex items-center justify-between gap-2 transition-all ${
                  isDone
                    ? 'bg-[#B8E986]/30 border-[#18181B]'
                    : isCurrent
                    ? 'bg-[#6C5CE7] text-white border-[#18181B] shadow-brutal-sm'
                    : 'bg-white text-[#18181B]/50 border-[#18181B]/20'
                }`}
              >
                <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                  <span className={`font-bold text-xs shrink-0 ${isCurrent ? 'text-white' : 'text-[#18181B]'}`}>
                    {step.id}
                  </span>
                  <div className="min-w-0">
                    <h3 className={`font-bold tracking-wider uppercase text-[11px] sm:text-xs truncate ${
                      isCurrent ? 'text-white font-display' : 'text-[#18181B]'
                    }`}>
                      {step.title}
                    </h3>
                    <p className={`text-[9px] sm:text-[10px] truncate ${isCurrent ? 'text-white/80' : 'text-[#18181B]/60'}`}>
                      {step.detail}
                    </p>
                  </div>
                </div>
                ...
                <div className="shrink-0 font-bold">
                  {isDone ? (
                    ...
                    <span className="w-5 h-5 sm:w-6 sm:h-6 bg-[#F3F1EC] text-[#18181B]/40 flex items-center justify-center border border-[#18181B]/20 text-[10px]">
                      ○
                    </span>
                  )}
                </div>
        ...
        <div className="pt-1 font-mono">
          <div className="flex justify-between text-[10px] sm:text-[11px] font-bold text-[#18181B] mb-1">
            <span>SYNTHESIS_PROGRESS</span>
            <span>{Math.round(((currentStepIndex + 1) / PIPELINE_STEPS.length) * 100)}%</span>
          </div>
          <div className="w-full h-2.5 sm:h-3 bg-white border-2 border-[#18181B] p-0.5">
            <div
